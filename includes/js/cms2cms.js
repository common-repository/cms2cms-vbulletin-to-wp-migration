if (typeof jQuery == 'undefined') {
    ...
    jQuery(document).ready(function () {
        if ($('.cms2cms-vBulletin-container').length > 0) {
            ...
            var init = function () {
                ...
                if ($('.active').hasClass('step-sign')) {
                    ...
                    $(document).on('click', '#verifySource_vBulletin', function (event) {
                ...
                $('#auth_submit').on('click', function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.key) {
                                saveUser({
                                    action: 'cms2cms_vBulletin_save_options',
                                    accessKey: data.key,
            ...
            var showErrors = function (errors) {
                $('.cms2cms-vBulletin-error').hide();
                for (var error in  errors) {
                    var message = (Object.values(errors[error])[0]) ? Object.values(errors[error])[0] : 'Invalid data';
                    $('.cms2cms-vBulletin-error.' + error + ' span').html(message).parent().fadeIn();
                }
            };
            ...
            var toggleSignIN = function () {
                var cms2cmsBlock = $('#cms2cms_vBulletin_accordeon');
                var signInTabs = cms2cmsBlock.find('a.nav-tab');

                signInTabs.on('click', function (e) {
                    if (!$(this).hasClass('cms2cms-vBulletin-real-link')) {
                        e.preventDefault();
                        var activeClass = 'nav-tab-active';
                        if (!$(this).hasClass(activeClass)) {
                            signInTabs.removeClass(activeClass);
                            $(this).addClass(activeClass);
                            $(this).closest('.cms2cms_vBulletin_accordeon_item').find('form').attr('action', $(this).attr('href'));
                        }
                    }
                });

                var formContent = $('.cms2cms_vBulletin_accordeon_item_register').find('.center-content'),
                    button = $('.login-reg');
                ...
                button.on('click', function () {
                    ...
                    $('.cms2cms-vBulletin-real-link').fadeToggle();
                    button.parent('p.account-register').toggle();
                    button.parent('p.account-login').toggle();
                    $('.cms2cms-vBulletin-error span').parent().fadeOut();
            ...
            var checkDoneTab = function () {
                ...
                if ($('.cms2cms-vBulletin-container').hasClass('cms2cms_vBulletin_is_activated')) {
            ...
            var validateSource = function () {
                ...
                switch (true) {
                    case $('#sourceUrl').val().length == 0:
                        $('.cms2cms-vBulletin-error span').html('Url can`t be empty').parent().fadeIn();
                        break;
                    case !regex.test(sourceUrl):
                        $('.cms2cms-vBulletin-error span').html('Url invalid, please enter url in "http://your_vBulletin_website.com/" format').parent().fadeIn();
                        break;
                    case sourceUrl == targetUrl:
                        $('.cms2cms-vBulletin-error span').html('Please, make sure to provide your source website URL (the URL of your current website)').parent().fadeIn();
                        break;
                    default :
                        $('#sourceUrl').val(sourceUrl);
                        ajaxValidate($('#verifySource_vBulletin').closest('form').serialize());
                        return;
                }
